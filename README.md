...
```sql
explain ANALYZE FORMAT=TREE SELECT * FROM ENTITY_WITH_AREA
        WHERE((ST_INTERSECTS(ST_BUFFER(ST_SRID(POINT(21.01098904782881, 52.22512499106378), 4326), 500 ), area)) AND tenant_id=1)
        ORDER BY ST_DISTANCE( ST_SRID(POINT(21.01098904782881, 52.22512499106378), 4326), ST_SRID(POINT(longitude, latitude), 4326)) ASC
```